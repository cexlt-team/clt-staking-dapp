...
contract CLTTokenWrapper {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IERC20 public stakeToken = IERC20(
        0x92B8B015a272c7734a378e68203780926986fb72
    );
...
contract CLTPool is CLTTokenWrapper, IRewardDistributionRecipient {
    IERC20 public clt = IERC20(0xFe42F80F0304305636F0B316d25CF76665c6086d);
    ...
    {
        require(amount > 0, "CLT-POOL: Cannot stake 0");
        super.stake(amount);
    ...
    function withdraw(uint256 amount)
        ...
        updateReward(msg.sender)
    {
        require(amount > 0, "CLT-POOL: Cannot withdraw 0");
    ...
    modifier checkStart() {
        require(block.timestamp > startTime, "CLT-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "CLT-POOL: Pool is closed");
        _;
    }
        ...
        updateReward(address(0))
    {
        if (block.timestamp > startTime) {
            ...
            uint256 b = clt.balanceOf(address(this));
            rewardRate = reward.add(b).div(DURATION);
            periodFinish = startTime.add(DURATION);
            lastUpdateTime = startTime;
        }

        clt.safeTransferFrom(msg.sender, address(this), reward);
